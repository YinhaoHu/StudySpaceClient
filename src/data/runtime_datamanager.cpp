...
update_personalInfo_profile()
{
	...
	personalInfo->userSmallProfile = new QPixmap(QPixmap(standard::file::selfProfile).scaled(32,32));
		
	if (personalInfo->userBigProfile != nullptr)
		delete personalInfo->userBigProfile;
	personalInfo->userBigProfile = new QPixmap(QPixmap(standard::file::selfProfile).scaled(100, 100));
...
void data::Runtime_DataManager::
add_friendsInfo_samllProfileObject(unsigned int id, QLabel* object)
{
...
update_friendsInfo_bigProfileObject(unsigned int id)
{
	...
	if (set.contains(id) == false)
		set[id] = new QPixmap();
	auto& data = *set[id];
	data = local_dataManager->getFriendProfile(id).scaled(standard::scale::bigProfileSideLen, standard::scale::bigProfileSideLen);
}
...
update_friendsInfo_smallProfileObject(unsigned int id)
{
	...
	if (set.contains(id) == false)
		set[id] = new QPixmap();
	auto& data = *set[id];
	data =local_dataManager->getFriendProfile(id).scaled(standard::scale::smallProfileSideLen, standard::scale::smallProfileSideLen);
}
